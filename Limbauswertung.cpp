					...
					Konfiguration &Konf)
{
	...
	for (i = 0; i < Rohdaten.size(); i++) { //Schleife über alle Rohdaten
		Messung_Limb Messung = Rohdaten[i];
...
//                }
				...
				Ausgewertete_Messung_Limb Ergebnis
					= Messung.Ergebnis_Zusammenfassen();
	...
	// Die Rohdaten werden nun nichtmehr gebraucht, sodass alle dynamisch
	// allokierten Felder gelöscht werden können
