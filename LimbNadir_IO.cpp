...
//////////////////////////////////////////////////////////////////////////////////////////////////////
int Load_Limb_Ascii(string Datei_in, string textheader[31], int &no_of_alt, int &no_of_pix, int Orbitstate[5], int Datum[6],
					float Center_Lat_Lon[10], float &orbit_phase, float*& Wellenlaengen, Limb_Datensatz*& Limbdaten)
{
	...
	int Anzahl_Big_Array = (no_of_alt + 1) * no_of_pix; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Gehe von 31 Höhen aus!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	float *Big_Array;
	...
	//
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	// Das ist zwar länglich und redundandant, aber vermutlich schneller als immer das ganze Feld zu betrachten
	// und hier wird das nur einmal gemacht, sodass es nicht bei jedem laden neu gemacht werden muss
...
int Load_Limb_l_mpl_binary(string Datei_in,
						   string textheader[31], int &no_of_alt, int &no_of_pix, int Orbitstate[5], int Datum[6],
						   float Center_Lat_Lon[10], float &orbit_phase, float*& Wellenlaengen, Limb_Datensatz*& Limbdaten)
{
	int lang_textheader = 31;
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int Load_Nadir_Ascii(string Datei_in,
					 ...
					 int*& Kanal_Nr, float*& Wellenlaenge, Nadir_Datensatz*& Nadirdaten)
{
	...
	//wir brauchen noch die Anzahl der Pixel...die für alle Messungen gleich sind
	for (int i = 0; i < 8; i++) {
	...
	for (int i = 0; i < No_of_Messungen; i++) {
		...
		infile >> s_dummy >> Nadirdaten[i].m_Messung_ID; //Zeile mit Messungsnummer...von 0 an durchnummeriert
		...
		infile >> Nadirdaten[i].m_Hoehe >> Nadirdaten[i].m_Sat_Lat
			   >> Nadirdaten[i].m_Sat_Lon >> Nadirdaten[i].m_Sat_Erdradius >> Nadirdaten[i].m_orbit_phase;
		...
		if (errortext != "ERRORS") {
			...
			cout << " Fehler in Load_Nadir_Ascii\n"
				 << "errortext ist nicht ERRORS... Datei in undefinierten Zustand\n";
			return 1;
		}
		// Fehlerfeld einlesen
		for (int j = 0; j < No_of_Pix; j++) {
							...
							int*& Kanal_Nr, float*& Wellenlaenge, Nadir_Datensatz*& Nadirdaten)
{
	...
	infile.open(Datei_in.c_str(), ios_base::binary);
	if (!infile.is_open()) {
		cout << Datei_in << "kann nicht zur Umwandlung in ASCII geöffnet werden\n";
		return 1;
	}
	...
	infile.read((char *) &No_of_Pix, sizeof(int));
	// Die wichtigsten Dimensionen der Felder sind bekannt....speicher allokieren
	Kanal_Nr = new int[No_of_Pix];
	Wellenlaenge = new float[No_of_Pix];
	Nadirdaten = new Nadir_Datensatz[No_of_Messungen];
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
					 ...
					 int *Kanal_Nr, float *Wellenlaenge, Nadir_Datensatz *Nadirdaten)
{
	...
	outfile.open(Datei_out.c_str());
	if (!outfile.is_open()) {
		cout << Datei_out << " kann nicht erzeugt werden in Nadir Rückumwandlung in Ascii\n";
		return 1;
	}
	for (int i = 0; i < Anzahl_Textheaderzeilen; i++) {
	...
	for (int i = 0; i < No_of_Messungen; i++) {
		...
		outfile << No_of_Pix << "\n";
		for (int j = 0; j < No_of_Pix; j++) {
			outfile << Kanal_Nr[j] << "\t" << Wellenlaenge[j] << "\t" << Nadirdaten[i].m_radiance[j] << "\n";
		}
		outfile << "ERRORS\n";
		for (int j = 0; j < No_of_Pix; j++) {
			outfile << Kanal_Nr[j] << "\t" << Wellenlaenge[j] << "\t" << Nadirdaten[i].m_error[j];
