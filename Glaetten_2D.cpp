...
int SCD_Glaettung(vector<Ausgewertete_Messung_Limb>& AML, int Anzahl_Linien, string limbmesothermo)
{
    ...
    int Anzahl_Hoehen;
    if(limbmesothermo=="ja")    {    Anzahl_Hoehen=25;/*25*/;    }
    else {    Anzahl_Hoehen=7;    }
    ...
    for( unsigned int i=0;i<AML.size();i++) {    AML[i].m_Zeilendichte=0;    }  // mit 0 resetten
    ...
    // Zuerst 4 Ecken Sonderbehandlung, dann 4 Kanten und schließlich den Rest der Matrix
    for( int k=0;k<Anzahl_Linien;k++)
    {
        ...
        // ECKE 1
        Messung_Nr=0+k;
        AML[Messung_Nr].m_Zeilendichte=(3.0*AML_OLD[Messung_Nr].m_Zeilendichte+
        ...
        AML[Messung_Nr].m_Fehler_Zeilendichten=(3.0*AML_OLD[Messung_Nr].m_Fehler_Zeilendichten+
