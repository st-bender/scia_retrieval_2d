...
/////////////////////////////////////////////////////////////////////////////////
void Fit_Polynom(double* x,double* y, int Startindex,int Endindex,double x0, int Polynomgrad, double*Parametervektor)
{
    // Fit eines Polynoms a0+a1x+a2x^2.......+anx^n    mit n=Polynomgrad
    // Dabei wird zunächst ein lineares Gleichungssystem aufgestellt
    // und dann gelöst  (Beispiel in Matlab für Polynom 4ten Grades, siehe unten....und Anleitung)
    ...
    for( int Index=Startindex;Index<=Endindex;Index++)
    {
        ...
        {
            LHS_Parameter_Zeile_eins_Inkrement[Par_Zeile]=LHS_Parameter_Zeile_eins_Inkrement[Par_Zeile-1]*h;
            RHS_Inkrement[Par_Zeile]=RHS_Inkrement[Par_Zeile-1]*h;
        }
        LHS_Parameter_letzte_Spalte_Inkrement[0]=LHS_Parameter_Zeile_eins_Inkrement[Polynomgrad];
        for(int Par_Spalte=1;Par_Spalte<Polynomgrad+1;Par_Spalte++)
        {    LHS_Parameter_letzte_Spalte_Inkrement[Par_Spalte]=LHS_Parameter_letzte_Spalte_Inkrement[Par_Spalte-1]*h;   }
        // Inkrementationen durchführen
        for(int i=0;i<Polynomgrad+1;i++)
        {
            LHS_Parameter_Zeile_eins[i]           =LHS_Parameter_Zeile_eins[i]+LHS_Parameter_Zeile_eins_Inkrement[i];
            LHS_Parameter_letzte_Spalte[i]       =LHS_Parameter_letzte_Spalte[i]+LHS_Parameter_letzte_Spalte_Inkrement[i];
    ...
    {
        LHS[Spaltennummer+Zeilennummer*(Polynomgrad+1)]=LHS_Parameter_Zeile_eins[Zeilennummer];
    ...
    {
        LHS[Spaltennummer+Zeilennummer*(Polynomgrad+1)]=LHS_Parameter_letzte_Spalte[Spaltennummer];
        //cout<<"Element_LHS: "<<Spaltennummer+Zeilennummer*(Polynomgrad+1)<<"\n";
    }
    // die erste Spalte und die letzte Zeile der Matrix sind nun besetzt
    // ab der 2ten Spalte die Spalte bis zur vorletzten Zeile durchgehen und das Element mit einer Spalte weniger und einer
    // Zeile mehr zuordnen (also das links unten)
    for(Spaltennummer=1;Spaltennummer<Polynomgrad+1;Spaltennummer++)
    {   for(Zeilennummer=0;Zeilennummer<Polynomgrad;Zeilennummer++)
        {
             LHS[Spaltennummer+Zeilennummer*(Polynomgrad+1)]=LHS[(Spaltennummer-1)+(Zeilennummer+1)*(Polynomgrad+1)];
             //cout<<"Element_LHS: "<<Spaltennummer+Zeilennummer*(Polynomgrad+1)<<"\n";
        }
    }
    ...
    int LDA=N;    int LDB=N;    int INFO;
    //Aufruf der LU-Zerlegung
    dgesv_(&N, &NRHS, LHS, &LDA, IPIV, RHS, &LDB, &INFO);
    // Lösungen liegen nun in RHS vor
    // Speicher für Parametervektor, wurde vor Funktionsaufruf (Fit_Polynom) reserviert
    for(int i=0;i<Polynomgrad+1;i++)
    ...
    // Geschieht über das minimieren der Summe der Quadrate der Abweichungen(Residuen)
    // der Fitfunktion von den Messpunkten
    // Sum( (f-y)^2)=min
    // Leitet man ab erhält man für jeden linearen Parameter....(das Wort linear dick unterstreichen)
    // Sum (f'f-f'y)=0  mit f'=df/da_i Ableitung nach linearem Parameter
    // in f' steckt a_i nichtmehr drin...sodass man f'y auf die andere Seite zieht alle Gleichungen f'y ergeben dann die Rechte Seite
    // von LHS x=RHS;
    // jede Summe f'f lässt sich als linearkombination der parameter a_i darstellen, sodass man so LHS x
    ...
    //for i=1:lang
    //    h=x(i)-mu;
    //    a0=a0+1;                         // bestimmen der parameter...bei jedem weiteren *h...das schreit nach Schleife
    //    b0=b0+h;                         //LHS
    //    c0=c0+h*h;
    //    d0=d0+h*h*h;
    //    e0=e0+h*h*h*h;
...
/////////////////////////////////////////////////////////////////////////////////
int x_zu_Minimum_von_y_in_Intervall(double* x, double* y,int Startindex,int Endindex,
                                                             double& x_min, double&y_min, int &Indexmin)
{
