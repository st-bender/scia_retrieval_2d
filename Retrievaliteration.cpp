                                  ...
                                  Konfiguration& Konf)
{
    ...
    // Debugging Output aus alter Routine wurde entfernt...kann von da aber hier rein kopiert werden

    MPL_Matrix LHS; //linke Seite der Gleichung(anzuwenden auf Dichten, um RHS zu erhalten)
...
//    cout<<"LHS: "<<LHS.m_Zeilenzahl<<"\t"<<LHS.m_Spaltenzahl<<"\n";
    ...
    // ich halte mich erstmal an Marcos Routine, bis auf die Diagonalisierung der Matrix...das mach ich mit Gauss statt cholesky,
    // da auch negative Dichten als Lösungen prinzipiell erstmal zugelassen werden sollen(das sagt einem ja dann auch was über die
    // Genauigkeit des Retrievals aus, ausserdem überschätzt man sonst die Dichte)
    ...
    // danach werden die Residuen berechnet und es wird überprüft, ob diese zum vorherigen Schritt kleiner geworden sind
    // falls nicht, wird abgebrochen
    // falls die Residuen kleiner geworden sind, werden alle positiven Einträge von X als neues apriori übergeben( die anderen bleiben gleich)

    //Der Solver löst die Gleichung mehrmals, falls IERR nicht 1 ist, d.h. die LHS nicht positiv definit, (dann wird Lambda_apriori
    //erhöht, was Unsinn ist, da das nur in die RHS einfließt, die keine Rolle bei der feststellung der Definitheit der LHS hat.
    //Das heißt, falls es kene Lösung gibt, braucht das Programm auch noch länger, da ja das Flag so nie 1 werden kann
    //Diesen Teil lass ich also weg und mach nur die Iteration
    //Nach meinem letzten Vortrag hieß es, dass eine derartige Anpassung des aprior auch nicht dichter an der Lösung liegt....
    //Evtl ist das apriori eher sowas wie der Iterationsanfang

    //Als Solver für das Programm werden die ATLAS/LAPACK Routinen für die LU-Zerlegung einer Matrix, und das Rückeinsetzen
    //mit hilfe dieser Zerlegung verwendet.
    //Diese implementation gehört zu den schnellsten ihrer art.
    //Es handelt sich dabei um 2 Schritte...die LU-Zerlegung selbst und die Rückeinsetzung
    //Die LU-Zerlegung braucht Zeit, das Rückeinsetzen erfolgt dagegen fast instantan
    ...
    // Zunächst die LU Zerlegung der LHS durchführen mit dummy RHS, in der Iteration dann nur das Rückeinsetzen nutzen
    // START LU ZERLEGUNG
    // VORBEREITEN
    MPL_Matrix A(LHS.m_Spaltenzahl,LHS.m_Zeilenzahl);
    A=LHS.transponiert(); //Fortran Matrizzen sind zu C++ Matrizzen transponiert
    ...
    // TODO im Prinizp muss man RHS, falls es eine Matrix ist, jetzt auch transponieren
    // ENDE LU ZERLEGUNG
    //cout<<"RHS.m_Zeilenzahl: "<<RHS.m_Zeilenzahl<<"\n";
    //////////////////////////////////////////////////////////////////////////////////
    ...
    Itmax=100;  //Die Schleife ist echt schnell...das sollten höchstens 10 sekunden sein
    ///////////////////////////////////////
    // erster Schritt
    ///////////////////////////////////////
    RHS=AMF_trans*(S_y*Saeulendichten)+S_apriori*Dichten_apriori; //RHS sollte ein Spaltenvektor sein
    // Lösungen durch Rückeinsetzen finden
    dgetrs_(textflag,&N, &NRHS, A.m_Elemente, &LDA, IPIV, RHS.m_Elemente, &LDB, &INFO);
    Dichten=RHS;
    ///////////////////////////////////////
    // ENDE erster Schritt
    ///////////////////////////////////////
    ...
    for(int Iterationsschritt=0;Iterationsschritt<Itmax;Iterationsschritt++)
    {
        ...
        if(Iterationsschritt==0)                    { Residual_1=Residual; } // erstes Residuum als ungefähre Fehlerabschätzung
        if(Residual< Threshold*Residual_1)  {    break;   }
...
//    cout<<"LHS: "<<LHS.m_Zeilenzahl<<"\t"<<LHS.m_Spaltenzahl<<"\n";
    ...
    Itmax=10000;  //Die Schleife ist echt schnell...das sollten höchstens 10 sekunden sein

    for(int Iterationsschritt=0;Iterationsschritt<Itmax;Iterationsschritt++)
    {

        //Auf der RHS ändert sich die Apriori-Lösung...d.h. neu berechnen
        RHS=RHS_Teil1+S_apriori*Dichten_apriori; //RHS sollte ein Spaltenvektor sein
        ...
        Differenz=Differenz.transponiert()*Differenz; //skalar als 1x1 Matrix getarnt

        double Summe_der_quadratischen_Abweichungen=Differenz(0);//
        //cout<<"Summe_der_quadratischen_Abweichungen: "<<Summe_der_quadratischen_Abweichungen<<"\n";
        //das ist jetzt sowas wie die Standardabweichung....der Nenner ist nur zur Normierung da
        //(spielt das noch irgendwo ne Rolle...zumindest nicht im Hauptprogramm) TODO herausfinden
        Residual=sqrt(Summe_der_quadratischen_Abweichungen);// /sqrt((double)LHS.m_Zeilenzahl-1);
        ...
        if(Residual< Threshold*Residual_1)
        {
            ...
            break; // achtung mit break und continue in for-schleifen(vor allem mit continue->(i++; continue;)
        }
